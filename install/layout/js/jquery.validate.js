    ...
    jQuery.fn.validate = function(options){
        ...
        if (options['live']) {
            ...
            if (jQuery(this).find('input').length > 0) {
                ...
                jQuery(this).find('input').bind('focus keypress click', function(){
                    jQuery("#" + SelfID).next('.' + options['error_message_class'].replace(/\s/g, ".")).remove();
                    jQuery("#" + SelfID).parents("." + options['error_container_class'].replace(/\s/g, ".")).removeClass('error');
                });
            }
            // if the passed in element is input to validate itselft
                ...
                jQuery(this).bind('focus keypress', function(){
                    ...
                    jQueryObjectBeforeErrorMessage.next('.' + options['error_message_class'].replace(/\s/g, ".")).fadeOut("fast", function(){
                        jQuery(this).remove();
                    });
    				jQuery(this).parents("." + options['error_container_class'].replace(/\s/g, ".")).removeClass('error');
                });
            }
        }
        ...
        function validate_field(id){
            ...
            if (!validation_state) {
                ...
                if (jQueryObjectBeforeErrorMessage.next('.' + options['error_message_class'].replace(/\s/g, ".")).length == 0) {

                    jQueryObjectBeforeErrorMessage.after('<span class="' + options['error_message_class'] + '">' + options['message'] + '</span>');
					          jQuery(id).parents("div." + options['error_container_class'].replace(/\s/g, ".")).addClass("error");
                }
